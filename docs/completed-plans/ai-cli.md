...
- `--priority <id>` optional; omit to use the board’s builtin **`none`** priority (value `0`)
...
- `--body-stdin`
- `--group <id>`
- `--priority <id>` (use the builtin **`none`** row id to reset to default)
- `--status <id>`
- `--list <id>`
- `--color <css-color>`
- `--clear-color`
...
- omitting `--priority` on `tasks add` omits `priorityId` in the POST body so the server assigns builtin **`none`**
- `--clear-color` writes `color: null`

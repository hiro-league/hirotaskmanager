...
function parseBrowserMode(browser: string | undefined): boolean | undefined {
  ...
  if (normalized === "manual" || normalized === "off" || normalized === "closed") {
    return false;
  }
...
> {
  ...
  return {
    ...
    data_dir: path.resolve(
      overrides.dataDir ?? existing.data_dir ?? getDefaultInstalledDataDir(configScope),
    ),
    auth_dir: path.resolve(existing.auth_dir ?? getDefaultInstalledAuthDir(configScope)),
...
async function runLauncherSetup(overrides: {
  ...
  const dataDirValue = await promptWithDefault("Data directory", defaults.data_dir);
    ...
    try {
      ...
      const dataDir = path.resolve(
        overrideDataDir ??
          ...
          getDefaultInstalledDataDir({ profile: selectedProfile, kind: "installed" }),
      );
      const shouldOpenBrowser = overrideOpenBrowser ?? launcherConfig.open_browser ?? true;

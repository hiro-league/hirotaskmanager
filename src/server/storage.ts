...
/** Resolve slug for a numeric board id. */
export async function slugForId(id: string): Promise<string | null> {
  const n = Number(id);
  if (!Number.isFinite(n)) return null;
  const db = getDb();
  const row = db
    .query("SELECT slug FROM board WHERE id = ?")
    .get(n) as { slug: string } | null;
...
/**
 * Monolithic PUT: upsert lists / groups / tasks by id so PKs stay stable (no delete-all-reinsert).
 */
function replaceBoardContent(
  db: Database,
  boardId: number,
  board: Board,
  slug: string,
): void {
  const view = normalizeBoardViewState(
    db,
    board.visibleStatuses,
    board.statusBandWeights,
  );
  board = { ...board, ...view };

  const now = board.updatedAt;
  db.run(
    "UPDATE board SET name = ?, slug = ?, updated_at = ? WHERE id = ?",
    [board.name, slug, now, boardId],
  );

  if (board.taskGroups.length === 0) {
    throw new Error("Board must have at least one task group");
  }

  const groupClientToDb = new Map<number, number>();
  const keptGroupIds = new Set<number>();

  for (const g of board.taskGroups) {
    if (g.id > 0) {
      const row = db
      ...
      if (row) {
        ...
        keptGroupIds.add(nid);
        groupClientToDb.set(g.id, nid);
      }
    } else {
      const r = db.run(
        "INSERT INTO task_group (board_id, label) VALUES (?, ?)",
  ...
  const listClientToDb = new Map<number, number>();
  const keptListIds = new Set<number>();
  ...
  for (const list of sortedLists) {
    if (list.id > 0) {
      const row = db
        ...
        .get(list.id, boardId) as { id: number } | null;
      if (row) {
        db.run(
          "UPDATE list SET name = ?, sort_order = ?, color = ? WHERE id = ?",
          [list.name, list.order, list.color ?? null, list.id],
        );
        keptListIds.add(list.id);
        listClientToDb.set(list.id, list.id);
      } else {
        const r = db.run(
        ...
        const nid = Number(r.lastInsertRowid);
        keptListIds.add(nid);
        listClientToDb.set(list.id, nid);
      }
    } else {
      const r = db.run(
        "INSERT INTO list (board_id, name, sort_order, color) VALUES (?, ?, ?, ?)",
  ...
  const existingLists = db
    ...
    .all(boardId) as { id: number }[];
  for (const { id: lid } of existingLists) {
    if (!keptListIds.has(lid)) {
      db.run("DELETE FROM list WHERE id = ?", [lid]);
    }
  }

  const incomingPositiveTaskIds = board.tasks
    .filter((t) => t.id > 0)
    .map((t) => t.id);
  if (incomingPositiveTaskIds.length === 0) {
    db.run("DELETE FROM task WHERE board_id = ?", [boardId]);
  } else {
    const ph = incomingPositiveTaskIds.map(() => "?").join(",");
    db.run(
      `DELETE FROM task WHERE board_id = ? AND id NOT IN (${ph})`,
      [boardId, ...incomingPositiveTaskIds],
    );
  }

  const allowedStatusIds = db
    .query("SELECT id FROM status ORDER BY sort_order ASC, id ASC")
    .all() as { id: string }[];

  for (const task of board.tasks) {
    const listId = listClientToDb.get(task.listId);
    const groupId = groupClientToDb.get(task.groupId);
    if (listId === undefined || groupId === undefined) {
      continue;
    }
    const statusId = coerceTaskStatus(
      task.status,
      allowedStatusIds.map((r) => r.id),
    );

    if (task.id > 0) {
      const exists = db
        .query("SELECT id FROM task WHERE id = ? AND board_id = ?")
        .get(task.id, boardId) as { id: number } | null;
      if (exists) {
        db.run(
  ...
  db.run(
    ...
    [
      boardId,
      JSON.stringify(board.visibleStatuses),
      board.statusBandWeights
        ? JSON.stringify(board.statusBandWeights)
        : null,
      board.boardLayout ?? null,
      board.boardColor ?? null,
      board.backgroundImage ?? null,
      board.showCounts ? 1 : 0,
    ],
  );
}

/** Full replace of lists / tasks / groups / view prefs for a board (monolithic PUT). */
export async function writeBoardAtomic(board: Board, slug: string): Promise<void> {
  const boardId = board.id;
  withTransaction(getDb(), () => {
    replaceBoardContent(getDb(), boardId, board, slug);
  });
}
...
export async function deleteBoardFile(slug: string): Promise<void> {
  const db = getDb();
  const row = db
    .query("SELECT id FROM board WHERE slug = ?")
    .get(slug) as { id: number } | null;
  if (!row) return;
  db.run("DELETE FROM board WHERE id = ?", [row.id]);
}

export async function renameBoardFile(oldSlug: string, newSlug: string): Promise<void> {
  if (oldSlug === newSlug) return;
  const db = getDb();
  db.run("UPDATE board SET slug = ? WHERE slug = ?", [newSlug, oldSlug]);
}
